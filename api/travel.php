	...
	public function exec(){
		if( preg_match('/(POST|GET|PUT|DELETE)/',$this->method) ){  // CHECK IF THE REQUESTED METHOD IS VALID
			if( preg_match('/POST/',$this->method) ){
				...
				$p->setCustomSQL("SELECT v.user,v.registration_number,v.registration_due_date,v.make,v.model,v.engine_type,l.date_begin,l.start_odometer_reading,l.start_location,l.date_finish,l.end_odometer_reading,l.end_location,l.kilometre_travelled,l.travel_type,l.purpose_of_the_journey from vehicle_details v, vehicle_logs l where v.registration_number=l.registration_number and v.registration_number = '".$this->travel."' ;");
